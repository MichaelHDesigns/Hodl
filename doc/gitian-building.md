...
*Setup instructions for a Gitian build of HodlCash using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the HodlCash
...
- [Building HodlCash](#building-hodlcash)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-hodlcash-host/README.md).
...
Clone the git repositories for hodlcash and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/hodlcash/hodlcash
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the hodlcash repository
...
To build HodlCash (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the hodlcash repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/hodlcash/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/hodlcash/hodlcash
...
For example:
```bash
URL=https://github.com/laanwj/hodlcash.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit hodlcash=${COMMIT} --url hodlcash=${URL} ../hodlcash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit hodlcash=${COMMIT} --url hodlcash=${URL} ../hodlcash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit hodlcash=${COMMIT} --url hodlcash=${URL} ../hodlcash/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hodlcash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hodlcash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/hodlcash-osx-build.assert
```
...
[hodlcash/gitian.sigs](https://github.com/hodlcash/gitian.sigs/) repository, or if that's not possible create a pull
