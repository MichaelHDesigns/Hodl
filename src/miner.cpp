...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
...

    {
        ...
        pblock->vtx[0] = txNew;
        pblocktemplate->vTxFees[0] = -nFees; // We could set UPPER CAP here of 120 HODL standard block reward 
...
{
    CHodlCashAddress addressParsed(address);
...
CBlockTemplate* CreateNewBlockWithAddress(std::string address)
{
    ...
    {
        scriptPubKey = GetScriptForDestination(CHodlCashAddress(address).Get());
    }
...
void static HodlCashMiner(CWallet *pwallet, uint32_t minerI, uint32_t minerN, int nThreads)
{
    ...
    LogPrintf("HodlCashMiner using address: [%s] with minerblocktimeout: %d\n",ma,tSleep);
    ...
    try {
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                LogPrintf("HodlCashMiner waiting for network peers...\n");
                do {
                      bool fvNodesEmpty;
                      {
                        LOCK(cs_vNodes);
                ...
                LogPrintf("HodlCashMiner network peers completed.\n");
            }

            LogPrintf("HodlCashMiner creating new block...\n");
            //
            // Create new block
            //
            ...
            try{
               if(ma!=""){
            	 if(validateAddress(ma)) {
                    LogPrintf("HodlCashMiner: Mining to User supplied address %s\n",ma);
                    pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithAddress(ma));
                 }else{
                    LogPrintf("HodlCashMiner: WARNING! User supplied address is invalid, defaulting to keypool address.\n");
                    pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithKey(reservekey));
                 }
               }else{
                 LogPrintf("HodlCashMiner: Mining to keypool address.\n");
                 pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithKey(reservekey));
              }
            } 
            catch (const std::runtime_error &e){
                LogPrintf("HodlCashMiner runtime error: %s\n ... retrying...", e.what());
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("HodlCashMiner: ERROR! Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
    ...
    {
        LogPrintf("HodlCashMiner terminated\n");
        aligned_free(scratchpad);
        fGenerate = false;
        return;
    }
...
void GenerateHodlCashs(bool fGenerate, CWallet* pwallet, int nThreads)
{
    if(isInterrupting){
        return;
    }
    ...
    for(int i=0;i<miners;i++){
        minerThreads->create_thread(boost::bind(&HodlCashMiner, pwallet, i, miners, nThreads));
        MilliSleep(200);
    }
}
