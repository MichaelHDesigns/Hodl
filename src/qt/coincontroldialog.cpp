    ...
    model(0)
{
    ...
    // see https://github.com/hodlcash/hodlcash/issues/5716
...
{
    GUIUtil::setClipboard(HodlCashUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = HodlCashUnits::HODL;
    ...
    l2->setText(HodlCashUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(HodlCashUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(HodlCashUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(HodlCashUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(HodlCashUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK())) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(HodlCashUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK()));

    QString toolTip3 = tr("This label turns red if any recipient receives an amount smaller than %1.").arg(HodlCashUnits::formatWithUnit(nDisplayUnit, ::minRelayTxFee.GetFee(546)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = QString::fromStdString(CHodlCashAddress(outputAddress).ToString());
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, HodlCashUnits::format(nDisplayUnit, withInterest));
            itemOutput->setText(COLUMN_INTEREST, HodlCashUnits::format(nDisplayUnit, withInterest-out.tx->vout[out.i].nValue));
            itemOutput->setText(COLUMN_INPUT, HodlCashUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_INTEREST, HodlCashUnits::format(nDisplayUnit, nSumInterest));
            itemWalletAddress->setText(COLUMN_INPUT, HodlCashUnits::format(nDisplayUnit, nSumInputs));
            itemWalletAddress->setText(COLUMN_AMOUNT, HodlCashUnits::format(nDisplayUnit, nSum));
