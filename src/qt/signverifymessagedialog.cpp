    ...
    model(0)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::hodlcashAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::hodlcashAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
	...
	LogPrintf("checking address for signmessage: [%s]\n", addressInput);
        ...
        CHodlCashAddress addr(addressInput);
        if (!addr.IsValid())
        {
            ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
...
{
    CHodlCashAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CHodlCashAddress(pubkey.GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
