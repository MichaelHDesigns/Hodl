	...
	else if (role == Qt::DisplayRole || role == SortRole || role == Qt::ToolTipRole)
	{
		...
		case Principal:
			if (role == SortRole)
				...
				return HodlCashUnits::format(nDisplayUnit, cache.nValue);
			break;
		case AccruedInterest:
			if (role == SortRole)
				return qint64(cache.withInterest-cache.nValue);
			else if (role == Qt::DisplayRole)
				return HodlCashUnits::format(nDisplayUnit, cache.withInterest-cache.nValue);
			break;
		case AccruedValue:
			if (role == SortRole)
				return qint64(cache.withInterest);
			else if (role == Qt::DisplayRole)
				return HodlCashUnits::format(nDisplayUnit, cache.withInterest);
			break;
		case OnMaturation:
			if (role == SortRole)
				return qint64(cache.matureValue);
			else if (role == Qt::DisplayRole)
				return HodlCashUnits::format(nDisplayUnit, cache.matureValue);
			break;
		case TermDays:
			if (role == SortRole)
