    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return HodlCashUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, HodlCashUnits::separatorNever);
            else
                return HodlCashUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    {
        amountTitle += " ("+HodlCashUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")";
    }
    return amountTitle;
}
