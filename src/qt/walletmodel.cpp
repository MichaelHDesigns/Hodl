...
{
    CHodlCashAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, std::string &termdepositquestion, int termDepositLength, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered hodlcash address / amount:
            ...
            if(termDepositLength>0){
                ...
                scriptPubKey = GetTimeLockScriptForDestination(CHodlCashAddress(rcp.address.toStdString()).Get(),chainActive.Height()+1+termDepositLength);
            }else{
                scriptPubKey = GetScriptForDestination(CHodlCashAddress(rcp.address.toStdString()).Get());
            }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal HodlCash:URI (HodlCash:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CHodlCashAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CHodlCashAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CHodlCashAddress(sAddress).Get();
