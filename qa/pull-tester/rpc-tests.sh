...
export HODLCASHCLI=${BUILDDIR}/qa/pull-tester/run-hodlcash-cli
export HODLCASHD=${REAL_HODLCASHD}
...
if [ "x${ENABLE_HODLCASHD}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and hodlcashd must all be enabled"
fi
